...
public class ArrayImpl<E> implements MyCollectionInterface<E>
{
   private E[] elements;
   ...
   private boolean grow()  {
	...
	public ArrayImpl<E> subList(int from, int to) {
		...
		if (from < num_elements && from >= 0 && to <= num_elements && to >= from) {
			ArrayImpl<E> returnArray = new ArrayImpl<E>(to-from);
				
			int temp =0;
			for (int i = from; i < to; i++) {
				returnArray.elements[temp] = elements[i];
				temp++;
				returnArray.num_elements=temp;
			}
